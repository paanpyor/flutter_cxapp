...
class _CSATPageState extends State<CSATPage> {
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            ...
            floating: false,
            pinned: true,
            backgroundColor: const Color(0xFFFFB84C).withOpacity(0.1),
            elevation: 0,
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: const Color(0xFFFFB84C).withOpacity(0.1),
                          borderRadius: BorderRadius.circular(20),
                        ),
                        child: Text(
                          "${answers.length}/${questions.length}",
                          style: const TextStyle(color: Color(0xFFFFB84C), fontWeight: FontWeight.bold),
                        ),
                      ),
                    ],
                  ),
  ...
  Future<void> _submitSurvey() async {
    ...
    await _db.child("users/${user.uid}/completedSurveys/${widget.restaurantId}").update({
      "completed": true,
      "date": now,
      "csat": avgScore, // <--- CRITICAL FIX
    });

    if (!mounted) return;
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Terima kasih atas maklum balas anda!"), backgroundColor: Color(0xFFFFB84C)),
    );
  }
        ...
        children: [
              ...
              Expanded(
                child: SliderTheme(
                  data: SliderTheme.of(context).copyWith(
                    ...
                    inactiveTrackColor: const Color(0xFFFFB84C).withOpacity(0.2),
                    thumbColor: const Color(0xFFFFB84C),
                    overlayColor: const Color(0xFFFFB84C).withOpacity(0.1),
                    trackHeight: 6,
              ...
              Container(
                ...
                child: Center(
                  child: Text(
                    ...
                    style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFFFFB84C)),
                  ),
                ),
              ),
            ],
          ...
          Padding(
            ...
            child: Text(
              ...
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFFFFB84C)),
            ),
          ),
        ],
      ),
